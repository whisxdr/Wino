...
use eframe::egui::{
    self, Color32, Frame, Margin, Pos2, Rect, RichText, Rounding, Stroke, TextEdit, Ui, Vec2,
};
...
pub fn start_two_columns(ui: &mut Ui, left_ratio: f32, gap: f32) -> (Ui, Ui, egui::Pos2, f32) {
    let total_w = ui.available_width();
    ...
    let left_rect =
        egui::Rect::from_min_size(initial_pos, Vec2::new(left_w, ui.available_height()));
...
pub fn metric_card(ui: &mut Ui, title: &str, value: &str, subtext: &str, accent_color: Color32) {
    let colors = get_colors(ui.visuals().dark_mode);
...
pub fn status_badge(ui: &mut Ui, text: &str, rgb: (u8, u8, u8)) {
    ...
    Frame::none()
        ...
        .stroke(Stroke::new(
            1.0_f32,
            Color32::from_rgba_unmultiplied(rgb.0, rgb.1, rgb.2, 80),
        ))
...
pub fn stat_row(ui: &mut Ui, label: &str, value: &str, is_highlight: bool) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.allocate_ui_with_layout(
            ...
            egui::Layout::right_to_left(egui::Align::Center),
            |ui| {
                let val_color = if is_highlight {
                    colors.accent
                } else {
                    colors.text_primary
                };
                ui.add(
                    egui::Label::new(RichText::new(value).size(11.5).strong().color(val_color))
                        .truncate(),
                );
            },
        );
...
pub fn live_histogram(ui: &mut Ui, samples: &[f32], max_val: f32, height: f32) {
    ...
    painter.rect_stroke(
        rect,
        Rounding::same(6.0),
        Stroke::new(1.0_f32, Color32::from_rgba_unmultiplied(49, 53, 61, 80)),
    );
