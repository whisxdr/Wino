use crate::app::components::{action_card, card_container, preview_group, status_badge, view_header, PreviewGroup};
use crate::app::state::AppState;
use crate::app::theme::get_colors;
use crate::app::worker::Action;
use crate::core::i18n::tr;
use crate::debloat::executor::is_rule_in_preset;
use eframe::egui::{Button, Color32, Frame, Margin, RichText, Rounding, Stroke, TextEdit, Ui, Vec2};
...
pub fn render(ui: &mut Ui, state: &mut AppState) {
    ...
    card_container(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            let safe_btn = Button::new(
                RichText::new(format!("{} ({} {})", tr(lang, "deb.safe_preset"), safe_count, tr(lang, "deb.rules_suffix")))
                    .color(if is_safe { Color32::WHITE } else { colors.text_primary }),
            )
            ...
            let balanced_btn = Button::new(
                RichText::new(format!("{} ({} {})", tr(lang, "deb.balanced_preset"), balanced_count, tr(lang, "deb.rules_suffix")))
                    .color(if is_balanced { Color32::WHITE } else { colors.text_primary }),
            )
            ...
            let agg_btn = Button::new(
                RichText::new(format!("{} ({} {})", tr(lang, "deb.aggressive_preset"), aggressive_count, tr(lang, "deb.rules_suffix")))
                    .color(if is_agg { Color32::WHITE } else { colors.text_primary }),
            )
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let apply_btn = Button::new(
                    RichText::new(format!("{} '{}'", tr(lang, "deb.apply_preset_btn"), state.debloat_preset))
                        .strong()
                        .color(Color32::WHITE),
                )
    ...
    if state.debloat_show_confirm {
        ...
        for item in &pending {
            for reg in &item.rule.registry_keys {
                reg_items.push(format!("[{}\\{}] {} = {}", reg.hive, reg.path, reg.value_name, reg.value_data));
            }
            ...
            .show(ui, |ui| {
                ui.vertical(|ui| {
                    ui.horizontal(|ui| {
                        ...
                        if pending_count == 0 {
                            ui.label(RichText::new(tr(lang, "deb.no_pending")).size(14.0).strong());
                        } else {
                            ui.label(
                                RichText::new(format!(
                                    "{}: '{}' ({} {})",
                    ...
                    if pending_count > 0 {
                        ui.add_space(6.0);
                        ui.label(RichText::new(tr(lang, "deb.preview_desc")).size(11.5).color(colors.text_secondary));
                        ui.add_space(6.0);
                        ...
                        ui.add_space(4.0);
                        ui.label(RichText::new(tr(lang, "deb.vss_note")).size(11.0).color(colors.secondary));
                        if pending_count > 0 {
                            ui.label(
                                RichText::new(
                                    if is_agg || confirm_preset == "Balanced" {
                    ...
                    ui.horizontal(|ui| {
                        if pending_count > 0 {
                            let confirm_btn = Button::new(RichText::new(tr(lang, "deb.confirm_apply_btn")).strong().color(Color32::WHITE))
                                .fill(modal_border)
                                .rounding(Rounding::same(6.0))
                        ...
                        let cancel_label = if pending_count == 0 { tr(lang, "common.close") } else { tr(lang, "deb.cancel_safe_btn") };
                        let cancel_btn = Button::new(RichText::new(cancel_label).color(colors.text_muted))
                            .fill(colors.bg_card_hover)
                            .rounding(Rounding::same(6.0))
    ...
    ui.horizontal(|ui| {
        ...
        for f in &["All", "Safe", "Balanced", "Aggressive"] {
            ...
            let btn = Button::new(*f)
                ...
                .stroke(Stroke::new(1.0_f32, if is_sel { colors.accent } else { colors.border }))
    ...
    for item in &state.debloat_items {
        ...
        action_card(
            ...
            |ui| {
                ...
                ui.add_space(2.0);
                ui.label(RichText::new(format!("{} {}", tr(lang, "common.benefit"), item.rule.estimated_benefit)).size(10.5).color(colors.text_muted));
            },
            |ui| {
                if item.is_applied {
                    ui.label(RichText::new(tr(lang, "common.optimized")).color(colors.success).strong());
                } else {
                    let btn = Button::new(RichText::new(tr(lang, "common.apply")).color(Color32::WHITE))
                        .fill(colors.accent)
                        .rounding(Rounding::same(6.0))
    ...
    if let Some(rule) = apply_item_rule {
        if state.action_busy {
            ...
            state.request_action(Action::ApplyDebloatRule(rule));
        }
    }
}
