use crate::app::components::{bento_card_sized, card_container, end_two_columns, progress_track_row, start_two_columns, stat_row, status_badge, view_header};
...
pub fn render(ui: &mut Ui, state: &mut AppState) {
    ...
    view_header(
        ui,
        tr(lang, "mem.title"),
        tr(lang, "mem.subtitle"),
        |ui| {
            status_badge(
                ui,
                &format!("{} {}", tr(lang, "mem.pressure_label"), state.memory_details.pressure.as_str()),
    ...
    bento_card_sized(&mut top_left_ui, tr(lang, "mem.phys_utilization"), "⚡", Some(tr(lang, "mem.hardware_badge")), Some(240.0), |ui| {
        ui.horizontal(|ui| {
            ui.vertical(|ui| {
                ui.label(RichText::new(format!("{:.1}%", state.metrics.ram_usage_pct)).size(36.0).strong().color(colors.accent));
                ui.label(RichText::new(format!("{:.1} GB / {:.1} GB {}", used_gb, total_gb, tr(lang, "mem.in_use_fmt"))).size(11.5).color(colors.text_secondary));
            });

            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ui.vertical(|ui| {
                    ui.label(RichText::new(format!("~{:.0} MB {}", cache_mb, tr(lang, "mem.standby_cached"))).size(13.0).strong().color(colors.secondary));
                    ui.label(RichText::new(tr(lang, "mem.cached_ready")).size(10.5).color(colors.text_muted));
                });
            });
        });

        ui.add_space(8.0);
        progress_track_row(ui, tr(lang, "mem.active_ram_load"), &format!("{:.1}%", state.metrics.ram_usage_pct), state.metrics.ram_usage_pct / 100.0, colors.accent);

        ui.add_space(6.0);
        stat_row(ui, tr(lang, "mem.available"), &format!("{:.1} GB", avail_gb), false);
        stat_row(ui, tr(lang, "mem.cached"), &format!("{:.0} MB", cache_mb), false);
        stat_row(ui, tr(lang, "mem.paged_pool"), &format!("{:.0} MB", paged_mb), false);
    });

    // Bento Right: Smart Optimization Action Card
    bento_card_sized(&mut top_right_ui, tr(lang, "mem.smart_optimization"), "🚀", None, Some(240.0), |ui| {
        ui.label(RichText::new(tr(lang, "mem.smart_optimization_desc")).size(11.5).color(colors.text_secondary));
        ui.add_space(8.0);
        ...
        Frame::none()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.label(RichText::new("ESTIMATED GAIN").size(10.0).color(colors.text_muted));
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
        ...
        let opt_label = if busy { tr(lang, "common.loading") } else { tr(lang, "mem.trim_now") };
