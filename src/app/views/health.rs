...
pub fn render(ui: &mut Ui, state: &mut AppState) {
    ...
    card_container(ui, |ui| {
        ui.horizontal(|ui| {
            ui.label(RichText::new("Overall System Health Rating").size(15.5).strong());
    ...
    card_container(ui, |ui| {
        ...
        ui.add_space(12.0);
        ui.horizontal(|ui| {
            let sfc_btn = Button::new(RichText::new("🔍 Run SFC Scan (/scannow)").color(colors.text_primary))
            ...
            let dism_btn = Button::new(RichText::new("🛠 Run DISM Health Check").color(colors.text_primary))
                .fill(colors.bg_card_hover)
                .rounding(Rounding::same(6.0))
                .min_size(Vec2::new(170.0, 30.0));
