...
pub fn render(ui: &mut Ui, state: &mut AppState) {
    ...
    view_header(ui, tr(lang, "ctx.title"), tr(lang, "ctx.subtitle"), |ui| {
    ...
    search_bar(
        ...
        "Filter context handlers by name, DLL, or CLSID...",
        Some(&count_str),
    );
    ...
    card_container(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ui.label(
                    RichText::new(tr(lang, "ctx.scan_hint"))
                        .size(12.0)
                        .color(colors.text_secondary),
                );
                ui.label(
                    RichText::new(tr(lang, "ctx.backup_note"))
                        .size(11.0)
    ...
    if state.context_menu_items.is_empty()
        && !state
            .pending_scans
            .contains(&crate::app::worker::ScanKind::ContextMenu)
    {
        card_container(ui, |ui| {
            ui.label(
                RichText::new(tr(lang, "ctx.none_found"))
                    .size(12.5)
                    .color(colors.text_muted),
            );
        });
        ...
        action_card(
            ...
            |ui| {
                ...
                ui.horizontal(|ui| {
                    status_badge(
                        ui,
                        &format!("{} {}", tr(lang, "ctx.root_badge"), entry.shell_root),
                        (100, 116, 139),
                    );
            ...
            |ui| {
                if entry.is_enabled {
                    let btn = Button::new(
                        RichText::new(tr(lang, "ctx.disable_btn")).color(Color32::WHITE),
                    )
                    ...
                    let btn = Button::new(
                        RichText::new(tr(lang, "ctx.enable_btn")).color(Color32::WHITE),
                    )
                    .fill(colors.accent)
                    .rounding(Rounding::same(6.0))
                    .min_size(Vec2::new(90.0, 26.0));
