use crate::app::components::{bento_card_sized, card_container, end_two_columns, live_histogram, progress_track_row, start_two_columns, stat_row, status_badge, view_header};
...
pub fn render(ui: &mut Ui, state: &mut AppState) {
    ...
    view_header(
        ...
        |ui| {
            let refresh_btn = Button::new(format!("↻ {}", tr(state.lang, "dash.refresh_telemetry")))
                .fill(colors.bg_card)
    ...
    bento_card_sized(&mut top_left_ui, tr(state.lang, "dash.cpu_telemetry"), "⚡", Some(tr(state.lang, "common.live_badge")), Some(210.0), |ui| {
        ui.horizontal(|ui| {
            ui.vertical(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ui.vertical(|ui| {
                    ...
                    ui.label(RichText::new(tr(state.lang, "dash.active_load")).size(10.5).color(colors.secondary));
                });
            });
        });
    ...
    bento_card_sized(&mut top_right_ui, tr(state.lang, "dash.host_info"), "🗖", None, Some(210.0), |ui| {
        let os_display = if state.sys_info.display_version.is_empty() {
            state.sys_info.os_name.clone()
        } else {
            format!("{} ({})", state.sys_info.os_name, state.sys_info.display_version)
        };
    ...
    // RAM Quick Card
    bento_card_sized(&mut mid_left_ui, tr(state.lang, "dash.memory_engine"), "⚡", Some("RAM"), Some(170.0), |ui| {
        ...
        ui.add_space(6.0);
        progress_track_row(ui, tr(state.lang, "dash.physical_utilization"), &format!("{:.1} {}", avail_gb, tr(state.lang, "dash.gb_available")), state.metrics.ram_usage_pct / 100.0, colors.accent);

        ui.add_space(4.0);
        ui.horizontal(|ui| {
            let busy = state.action_busy;
            let trim_label = if busy { tr(state.lang, "common.loading") } else { tr(state.lang, "dash.quick_trim") };
            let trim_btn = Button::new(RichText::new(trim_label).strong().color(Color32::WHITE))
                .fill(colors.accent)
                .rounding(Rounding::same(6.0))
    ...
    // Storage Quick Card
    bento_card_sized(&mut mid_right_ui, tr(state.lang, "dash.storage_status"), "🗑", Some("DRIVE"), Some(170.0), |ui| {
        ...
        ui.add_space(6.0);
        progress_track_row(ui, tr(state.lang, "dash.system_volume_usage"), &format!("{:.1} {}", total_gb, tr(state.lang, "dash.gb_total")), state.metrics.disk_usage_pct / 100.0, colors.secondary);

        ui.add_space(4.0);
        ui.horizontal(|ui| {
            let clean_btn = Button::new(RichText::new(tr(state.lang, "dash.scan_junk")).color(colors.text_primary))
    ...
    card_container(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            ui.label(RichText::new(tr(state.lang, "dash.health_actions")).size(15.0).strong());
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    if state.health_report.defender_active {
                        ...
                        ui.vertical(|ui| {
                            ui.label(RichText::new(tr(state.lang, "dash.av_active_title")).size(13.0).strong().color(colors.secondary));
                            ui.label(RichText::new(tr(state.lang, "dash.av_active_desc")).size(11.0).color(colors.text_secondary));
                        });
                    } else {
                        ui.label(RichText::new("⚠️").size(16.0).color(colors.danger));
                        ui.vertical(|ui| {
                            ui.label(RichText::new(tr(state.lang, "dash.av_off_title")).size(13.0).strong().color(colors.danger));
                            ui.label(RichText::new(tr(state.lang, "dash.av_off_desc")).size(11.0).color(colors.text_secondary));
                        });
                    }
