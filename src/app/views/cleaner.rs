...
pub fn render(ui: &mut Ui, state: &mut AppState) {
    ...
    card_container(ui, |ui| {
        ui.horizontal(|ui| {
            ui.vertical(|ui| {
                ui.label(
                    RichText::new(tr(lang, "dash.recoverable_space"))
                        .size(13.0)
                ...
                ui.label(
                    RichText::new(format!("{} {}", total_files, tr(lang, "clean.files_ready")))
                        .size(11.0)
                        .color(colors.text_muted),
                );
            });
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let label = if state.action_busy {
                    tr(lang, "common.loading")
                } else {
                    tr(lang, "clean.clean_now")
                };
                ...
                if ui
                    .add_enabled(
                        !state.action_busy && !state.cleaner_items.is_empty(),
                        clean_btn,
                    )
                    .clicked()
                {
                ...
                ui.vertical(|ui| {
                    ui.horizontal(|ui| {
                        ...
                        status_badge(
                            ...
                            &format!(
                                "{:.1} {}",
                                item.total_bytes as f64 / (1024.0 * 1024.0),
                                tr(lang, "clean.mb_size")
                            ),
                            (34, 197, 94),
                        );
                    });
