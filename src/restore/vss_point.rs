...
use windows::Win32::System::Registry::{RegCloseKey, RegQueryValueExW, RegSetValueExW, HKEY_LOCAL_MACHINE, KEY_READ, KEY_WRITE, REG_DWORD};
...
type SrSetRestorePointFn = unsafe extern "system" fn(*mut RestorePointInfoW, *mut StateMgrStatus) -> i32;
...
fn read_creation_frequency() -> Option<u32> {
    ...
    let name_w: Vec<u16> = "SystemRestorePointCreationFrequency".encode_utf16().chain(std::iter::once(0)).collect();
    ...
    unsafe {
        ...
        let opened = RegOpenKeyExW(HKEY_LOCAL_MACHINE, PCWSTR(path_w.as_ptr()), 0, KEY_READ, &mut key);
        if opened.is_err() {
            return None;
        }
        let res = RegQueryValueExW(key, PCWSTR(name_w.as_ptr()), None, None, Some((&mut value as *mut u32).cast()), Some(&mut size));
        let _ = RegCloseKey(key);
        if res.is_ok() { Some(value) } else { None }
    }
}
...
impl FrequencyGuard {
    fn engage() -> Self {
        ...
        unsafe {
            ...
            let mut key = HKEY_LOCAL_MACHINE;
            if RegCreateKeyExW(HKEY_LOCAL_MACHINE, PCWSTR(path_w.as_ptr()), 0, None, REG_OPTION_NON_VOLATILE, KEY_WRITE | KEY_READ, None, &mut key, None).is_ok() {
...
impl Drop for FrequencyGuard {
    fn drop(&mut self) {
        ...
        unsafe {
            use windows::Win32::System::Registry::{RegCreateKeyExW, RegDeleteValueW, REG_OPTION_NON_VOLATILE};
            let mut key = HKEY_LOCAL_MACHINE;
            if RegCreateKeyExW(HKEY_LOCAL_MACHINE, PCWSTR(path_w.as_ptr()), 0, None, REG_OPTION_NON_VOLATILE, KEY_WRITE | KEY_READ, None, &mut key, None).is_ok() {
                match self.previous {
                    Some(mins) => {
                        let bytes = mins.to_ne_bytes();
                        let _ = RegSetValueExW(key, PCWSTR(name_w.as_ptr()), 0, REG_DWORD, Some(&bytes));
                    }
                    None => {
...
pub fn create_windows_restore_point(description: &str) -> Result<String, String> {
    log_info("restore", &format!("Creating native Windows System Restore Point via VSS: '{}'", description));
    ...
    let mut status = StateMgrStatus { dw_status: 0, ll_sequence_number: 0 };
    ...
    let mut end_status = StateMgrStatus { dw_status: 0, ll_sequence_number: 0 };
    let _ = unsafe { sr_set_restore_point(&mut end_info, &mut end_status) };

    let msg = format!("Windows System Restore Point created successfully (sequence {}).", sequence);
