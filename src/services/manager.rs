...
pub fn set_service_startup(service_name: &str, startup: StartupType) -> Result<(), String> {
    ...
    if matches!(
        ...
        log_warn(
            "services",
            &format!(
                "BLOCKED: Attempted to modify critical system service: {}",
                service_name
            ),
        );
        return Err(format!(
            "Modification of critical system service '{}' is prohibited for stability.",
            service_name
        ));
    }
    ...
    let svc_name_wide: Vec<u16> = service_name
        .encode_utf16()
    ...
    unsafe {
        ...
        let Ok(scm) = scm_res else {
            return Err(
                "Failed to open Service Control Manager (Administrator rights required)."
                    .to_string(),
            );
        };

        let svc_res = OpenServiceW(scm, PCWSTR(svc_name_wide.as_ptr()), SERVICE_CHANGE_CONFIG);

        let Ok(svc) = svc_res else {
            let _ = CloseServiceHandle(scm);
            return Err(format!(
                "Failed to open service '{}'. Access denied or service not found.",
                service_name
            ));
        };
        ...
        if res.is_ok() {
            log_info(
                "services",
                &format!("Changed startup type for {} to {:?}", service_name, startup),
            );
            Ok(())
        } else {
            log_error(
                "services",
                &format!("Failed to change startup type for {}", service_name),
            );
            Err(format!(
                "Failed to update service config for {}",
                service_name
            ))
        }
    }
